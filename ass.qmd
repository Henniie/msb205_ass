...
# Laster inn GeoPackage-filen
kc_house_data_7654 <- st_read("MapsData/kc_house_data_7654.gpkg")

# Sjekker de første radene av dataene
...
## iv. Modellen skal inneholde områdevariablene familie inntekt og variabelen som angir etnisitet (rase).

```{r}
# formel
mod_formula <- price ~ sqft_living + bedrooms + bathrooms + floors + yr_built + 
               inc_fam_tot + POC_percen + year_month
...
# Moran I-test residualene
moran_test <- lm.morantest(model_ols, listw)
print(moran_test)
```

```{r}
# Visualiser romlig autokorrelasjon i residualene
moran.plot(residuals_ols, listw)
```
...
listw_k3 <- nb2listw(nb_k3, style = "W")  # "W" for vektet matrise

# Vekmatrise for de 10 nærmeste naboene
...
I ett boligmarked kan det være både blanding av lokale og globale effekter. En lokal tilnærming ser på nabolagseffekter, mens globale fenomen er når markedsforholdene påvirker en hel region. I dette tilfellet ... slide 27.
...
```{r}
# durban formel
durban_vars <- as.formula(~ sqft_living + bedrooms + bathrooms + floors + yr_built + inc_fam_tot + POC_percen)

# SDM modell
sdm_model <- lagsarlm(mod_formula, data = kc_house_data_with_tracts, listw = listw, type = "Durbin")
