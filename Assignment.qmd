...
# ag: Tror dere havner i trøbbel her. Jeg ville heller bare brukt 
# en left_join med x = tracts10 og y = income_data. I tract10 har 
# dere en variabel GEO_ID_TRT med ID for census tract. Tilsvarende
# variabel i income_data heter GEOIDTRT.
# Følgende kode vil legg inntektsdataene til tract10
...
tracts10 <- left_join(
  x = tracts10,
  # ag: er vel bare perc_low_income:perc_high_income vi trenger
  y = select(income_data, GEOIDTRT, perc_low_income:perc_high_income),
  by = join_by(GEO_ID_TRT == GEOIDTRT)
)
...
# 
# ag: Trenger ikke gjøre en spatial join for begge er samme type 
# kart over de 398 census tract og vi har en kode for å knytte dem sammen
# men vi dropper geometrien fra kc_wadoh_map slik at vi bare legger en
# dataframe med data til tracts10. Vi matcher igjen med tract ID.
# kc_wadoh_map <- st_join(kc_wadoh_map, income_data_sf, by = c("GEO_ID_TRT" = "GEOIDTRT"))

tracts10 <- left_join(
  x = tracts10,
  y = st_drop_geometry(kc_wadoh_map),
  # ID har samme navn i de to filene
  by = join_by(GEO_ID_TRT)
)

# ag: Da er vi ferdige og har fått dataene inn i tracts10. Dataene for rase
# ser det ut til at dere har droppet, men de kunne vært lagt inn på samme 
# vis som inntektsdataene
...
```{# Konverter income_data til sf}
#| eval: false
# Dette er ikke nødvendig. En vanlig left_join() gjør jobben.
income_data_sf <- income_data %>%
  mutate(dummy_geometry = st_sfc(st_point(c(NA_real_, NA_real_)))) %>%
  st_as_sf()
...
#| echo: false
#| output: false
# Her kreves en spatial join siden husene er punktdata med sine variabler, 
# mens dataene i tracts10 er område data. Vi ønsker å tilordne til husene 
# data for området de ligger i. Dette gjøres ved en spatial join der vi
# finner ut hvilke punkter som ligger i hvilket område. Her viktig at 
# projeksjonene er de samme.
...
```{r}
tracts10 <- st_transform(tracts10, "EPSG:2926")
```
...
```{r}
# Oppsummere
# ag: Summary og tolkning av koeffisientene er bare relevant for SEM og OLS. 
# for SDEM og SLX er det impacts som skal tolkes
#summary(sdem_model)
#summary(slx_model)
summary(sem_model)
summary(ols_model)
...
ag: Summary og impacts bør kanskje tolkes før dere tester for å finne den beste modellen. Event. hvis en vil gjøre det kotfattet kan en teste for å bestemme beste modell og så tolke summary(hvis SEM eller OLS er best) eller impacts (hvis SDEM eller SLX er best).
